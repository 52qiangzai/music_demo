<template>
  <div class="play">
    ...
    <div
      class="song-bg"
      :style="`background-image: url(${songInfo.picUrl}?imageView&thumbnail=360y360&quality=75&tostatic=0);`"
    ></div>
    <!-- 播放页头部导航 -->
    <div class="header">
    ...
    <div class="song-wrapper">
      ...
      >
        <div class="song-img">
          <img
            style="width: 100%"
            :src="`${songInfo.picUrl}?imageView&thumbnail=360y360&quality=75&tostatic=0`"
            alt=""
          />
        </div>
      </div>
      ...
      <div class="song-msg">
        <!-- 歌曲名 -->
        <h2 class="m-song-h2">
          <span class="m-song-sname">{{ songInfo.name }}</span>
        </h2>
        <!-- 歌词部分-随着时间切换展示一句歌词 -->
...
export default {
  ...
  methods: {
    ...
    async getMusicLyric() {
      // 获取歌曲详情, 和歌词方法
      const res = await reqMusicLyric(this.id);
      if (res.code === 200) {
        this.lyric = this._formatLyr(res.lrc.lyric);
      }
      // 初始化完毕先显示零秒歌词
